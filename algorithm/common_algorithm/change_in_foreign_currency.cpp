...
bool canGetChange( int targetMoney, vector<int>& denominations, int curIndex ) {
    if( targetMoney == 0 ) {
        return true;
    }
    if( curIndex == denominations.size() ) {
        return false;
    }
    int money = denominations[curIndex];
    if( money > targetMoney ) {
        return false;
    }
    if( targetMoney % money == 0 ) {
       return true;
    }
    int maxCount = targetMoney / money;
    for( int use = 0; use <= maxCount; use++ ) {
        bool can_change = canGetChange(targetMoney-use*money, denominations, curIndex+1);
        if( can_change ) {
            return true;
        }
    }
...
bool canGetExactChange(int targetMoney, vector<int>& denominations){
  ...
  return canGetChange(targetMoney,denominations,0);
}
